...
export function applyBattleCommand(scenario: BattleScenario, state: BattleState, command: BattleCommand): BattleState {
  ...
  switch (command.type) {
    ...
    case "MOVE_UNIT": {
      const unit = state.units[command.unitId];
      if (!unit) return addLog(state, "Отряд не найден.", "danger");
      if (unit.sideId !== state.activeSideId) return addLog(state, "Сейчас ход противника.", "danger");
    ...
    case "ATTACK_UNIT": {
      ...
      if (attacker.sideId !== state.activeSideId) return addLog(state, "Сейчас ход противника.", "danger");
      ...
      if (target.status === "destroyed" || target.status === "routed") return addLog(state, "Цель уже выведена из боя.", "danger");
      const preview = getCombatPreview(scenario, state, attacker, target);
      if (!preview.canAttack) return addLog(state, preview.reason ?? "Атака невозможна.", "danger");
      const result = resolveCombat(scenario, state, attacker, target);
      if (!result.canAttack) return addLog(state, result.reason ?? "Атака невозможна.", "danger");
      const updatedTarget: Unit = { ...target, currentHp: result.targetHpAfter, currentCount: result.targetCountAfter, morale: Math.max(0, target.morale - result.moraleDamage) };
      updatedTarget.status = result.targetDestroyed ? "destroyed" : result.targetRouted ? "routed" : getMoraleStatus(updatedTarget);
      const updatedAttacker: Unit = { ...attacker, hasAttacked: true };
      const nextState: BattleState = { ...state, selectedUnitId: updatedAttacker.status === "destroyed" || updatedAttacker.status === "routed" ? null : state.selectedUnitId, units: { ...state.units, [attacker.id]: updatedAttacker, [target.id]: updatedTarget } };
      return finishIfNeeded(scenario, addLog(nextState, result.logText, result.targetDestroyed || result.targetRouted ? "success" : "info"));
    }
    case "END_TURN": return finishIfNeeded(scenario, endTurn(scenario, state));
  }
}
...
export function endTurn(scenario: BattleScenario, state: BattleState): BattleState {
  ...
  const units = Object.fromEntries(Object.values(state.units).map((unit) => {
    if (unit.status === "destroyed" || unit.status === "routed") return [unit.id, { ...unit, hasMoved: false, hasAttacked: false }];
    ...
    return [unit.id, { ...unit, morale: recoveredMorale, status: status === "skipping" ? "shaken" : status, hasMoved: false, hasAttacked: false }];
  }));
  let nextState = addLog({ ...state, turn: nextTurn, activeSideId: nextSide.id, currentStageId: currentStage, selectedUnitId: null, units }, `Ход переходит к стороне: ${nextSide.name}.`, "history");
...
export function runBasicAiTurn(scenario: BattleScenario, state: BattleState): BattleState {
  let nextState = state;
  const aiUnits = Object.values(nextState.units).filter((unit) => unit.sideId === nextState.activeSideId && canUnitReceiveOrders(unit));
  for (const unit of aiUnits) {
    ...
    if (bestMove) nextState = applyBattleCommand(scenario, nextState, { type: "MOVE_UNIT", unitId: refreshedAfterAttack.id, to: bestMove });
  }
...
function finishIfNeeded(scenario: BattleScenario, state: BattleState): BattleState {
  ...
  return { ...state, phase: "finished", result, selectedUnitId: null };
}

function getCurrentStageId(scenario: BattleScenario, turn: number): string {
  return [...scenario.stages].sort((a, b) => b.startsAtTurn - a.startsAtTurn).find((stage) => turn >= stage.startsAtTurn)?.id ?? scenario.stages[0]?.id ?? "main";
}

function addLog(state: BattleState, text: string, tone: LogTone): BattleState {
  return { ...state, log: [{ id: `${state.turn}-${state.log.length}-${Math.random().toString(16).slice(2)}`, turn: state.turn, text, tone }, ...state.log].slice(0, 14) };
}
